...
export default function AddProduct() {
    ...
    const saveChanges = async () => {
        ...
        if (isValid) {
            ...
            const data = await postProduct(copyProduct);
            if (data.status == 201) {
                setProduct({
                    name: "",
                    description: "",
                ...
                <ScrollView>
                    ...
                    <Pressable style={[styles.filterContainer, { zIndex: 2 }]} onPress={() => { setShowedCategory(!isShowedCategory) }} >
                        <View style={styles.filterWrapper}>
                            <Text style={[
                                styles.inputFilter,
                                { color: `${product.category.name === "" ? "grey" : "black"}` },
                                errors.category ? styles.inputError : null
                            ]}>
                                {product.category.name === "" ? "Thể loại" : product.category.name}
                            </Text>
                            <View style={styles.iconFilterContainer}>
                                <AntDesign name={isShowedCategory ? "caretup" : "caretdown"} size={15} color={errors.category ? "red" : "grey"} />
                            </View>
                        </View>
                        {isShowedCategory &&
                            <View style={styles.showItem}>
                                {
                                    arrCategoriesData.map((category, index) => (
                                        <Pressable
                                            style={[styles.item, {
                                                ...
                                                backgroundColor: product.category._id === category._id ? "#FFBE98" : "#FEECE2",
                                                borderTopWidth: index == 0 || product.category._id === category._id ? 1 : 0,
                                                borderBottomWidth: index == arrCategoriesData.length - 1 || product.category._id === category._id ? 1 : 0,
                                                borderLeftWidth: 1,
                                                borderRightWidth: 1,
                                            }]}
                                            onPress={() => {
                                            ...
                                            key={index}
                                        >
                                            <Text style={{ textAlign: "left", textAlignVertical: "center", paddingLeft: 10 }}>{category.name}</Text>
                                        </Pressable>
                                    ))
                                }
                            </View>
                        ...
                        <View style={styles.inputWrapper}>
                            {
                                ...
                                >{errorsString.importedDate}</Text>
                            }
                        </View>
                    </View>
